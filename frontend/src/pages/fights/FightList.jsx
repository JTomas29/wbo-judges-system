...
const FightList = () => {
  ...
  useEffect(() => {
    ...
    getFights(token)
      ...
      .catch((err) => {
        setError(err.response?.data?.message || 'Error al cargar peleas');
        setLoading(false);
      });
  }, [token]);
            ...
            {fights.map((fight) => (
              <tr key={fight.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <td className="py-3 px-4">
                  <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${statusStyle(fight.status)}`}>
                    {statusLabel(fight.status)}
                  </span>
                </td>
                <td className="py-3 px-4 text-gray-600 whitespace-nowrap">
                  {fight.confirmed_judges} / {fight.min_judges_required}
                </td>
                <td className="py-3 px-4">
