...
const FightCard = ({ fight, onView, onEdit, canEditFlag }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 transition-all duration-200 hover:shadow-md">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-500">{fight.weight_class || fight.venue || '\u2014'}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
        ...
        <div>
          ...
          <p className="text-sm font-medium text-slate-800">{formatDate(fight.scheduled_date)}</p>
        </div>
        <div>
          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-0.5">Jueces</p>
          <div className="flex items-center gap-2">
            <span className="text-sm font-bold text-slate-800">{fight.confirmed_judges}/{fight.min_judges_required}</span>
      ...
      <div className="flex gap-2 pt-3 border-t border-slate-100">
        <button onClick={() => onView(fight.id)} className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-50 text-blue-700 rounded-xl text-xs font-semibold hover:bg-blue-100 transition-all active:scale-[0.97]">
  ...
  if (loading) return (
    <div className="flex items-center justify-center py-20">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-slate-500 font-medium">Cargando peleas...</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          <h1 className="text-[28px] sm:text-[34px] font-extrabold text-slate-900 tracking-tight leading-tight">Listado de Peleas</h1>
          <p className="text-sm text-slate-500 mt-1">Administra, consulta y gestiona todas las peleas registradas.</p>
        </div>
        <button
      ...
      {filteredFights.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-bold text-slate-800 mb-2">No se encontraron peleas</h3>
          <p className="text-sm text-slate-500 mb-6">Prueba modificando los filtros o crea una nueva pelea.</p>
          <button
            onClick={() => navigate('/fights/create')}
          ...
          <div className="hidden sm:block bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md">
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-white z-10">
                  <tr className="border-b border-slate-100">
                    ...
                    <th className="text-left py-4 px-5 text-[11px] font-semibold text-slate-400 uppercase tracking-wide hidden xl:table-cell">Árbitro</th>
                    <th className="text-left py-4 px-5 text-[11px] font-semibold text-slate-400 uppercase tracking-wide">Estado</th>
                    <th className="text-center py-4 px-5 text-[11px] font-semibold text-slate-400 uppercase tracking-wide hidden lg:table-cell">Jueces</th>
                    <th className="text-right py-4 px-5 text-[11px] font-semibold text-slate-400 uppercase tracking-wide">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredFights.map((fight, i) => {
                    ...
                    return (
                      <tr
                        key={fight.id}
                        className={`border-b border-slate-50 hover:bg-red-50/40 transition-colors cursor-pointer ${i === filteredFights.length - 1 ? 'border-0' : ''}`}
                      ...
                      >
                        <td className="py-4 px-5">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-bold text-slate-900 truncate max-w-[200px]">{fight.event_name}</p>
                              <p className="text-xs text-slate-400">{fight.weight_class || fight.venue || '\u2014'}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-5">
                        ...
                        <td className="py-4 px-5 text-slate-600 whitespace-nowrap hidden lg:table-cell text-xs">
                        ...
                        <td className="py-4 px-5 hidden xl:table-cell text-slate-600 text-xs">
                          {fight.referee_name || '\u2014'}
                        </td>
                        ...
                        <td className="py-4 px-5 text-right" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-end gap-2">
                            <button
                              onClick={() => navigate(`/fights/${fight.id}`)}
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-blue-50 text-blue-700 rounded-xl hover:bg-blue-100 hover:shadow-sm transition-all active:scale-[0.97]"
                            >
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                              ...
                              <button
                                ...
                                className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-xl transition-all active:scale-[0.97] ${
                                  canEdit(fight.status)
                                    ? 'bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 hover:border-slate-400 hover:shadow-sm'
                                    : 'bg-slate-50 border border-slate-200 text-slate-400 cursor-not-allowed'
                                }`}
            ...
            <div className="px-5 py-4 border-t border-slate-100 flex items-center justify-between">
              <p className="text-xs text-slate-400">
                Mostrando <span className="font-semibold text-slate-600">{filteredFights.length}</span> de{' '}
                <span className="font-semibold text-slate-600">{fights.length}</span> peleas
              </p>
