...
const formatDate = (d) => d ? new Date(d).toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit', year: 'numeric' }) : 'â€”';
const InfoRow = ({ label, value }) => (
  <div><label className="block text-xs text-gray-400 uppercase tracking-wider mb-0.5">{label}</label><span className="text-base font-semibold text-gray-800">{value || 'â€”'}</span></div>
);
              ...
              <tbody>
                {fight.assigned_judges.map((j) => (
                  <tr key={j.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-2 px-3"><span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${levelLabel(j.level)}`}>{j.level || 'â€”'}</span></td>
      ...
      {isStaff && (
        <div className="bg-white rounded-xl shadow-sm p-5 mb-6">
          ...
          <div className="flex gap-3 flex-wrap">
            <button className="inline-flex items-center justify-center px-5 py-2.5 bg-[#6b1421] text-white rounded-lg text-sm font-semibold hover:bg-[#4a0f14] transition-colors disabled:opacity-40 disabled:cursor-not-allowed" disabled={fight.status !== 'active'}>Asignar Jueces</button>
            <button className="inline-flex items-center justify-center px-5 py-2.5 bg-[#6b1421] text-white rounded-lg text-sm font-semibold hover:bg-[#4a0f14] transition-colors disabled:opacity-40 disabled:cursor-not-allowed" disabled={fight.status !== 'active'} title={fight.status !== 'active' ? 'La pelea debe estar activa' : ''}>Finalizar Pelea</button>
