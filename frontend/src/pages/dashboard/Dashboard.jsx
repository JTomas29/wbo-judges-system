...
const statusColors = {
  ...
  active: { bg: 'bg-emerald-50 dark:bg-emerald-900/30', text: 'text-emerald-700 dark:text-emerald-300', dot: 'bg-emerald-500', label: 'Active', icon: 'zap', border: 'border-emerald-200 dark:border-emerald-800/50' },
  completed: { bg: 'bg-blue-50 dark:bg-blue-900/30', text: 'text-blue-700 dark:text-blue-300', dot: 'bg-blue-500', label: 'Completed', icon: 'check', border: 'border-blue-200 dark:border-blue-800/50' },
  cancelled: { bg: 'bg-red-50 dark:bg-red-900/30', text: 'text-red-700 dark:text-red-300', dot: 'bg-red-500', label: 'Cancelled', icon: 'x', border: 'border-red-200 dark:border-red-800/50' },
};
...
const Dashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setError(err.response?.data?.message || 'Error loading the dashboard');
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h3 className="text-lg font-bold text-slate-800 mb-2 dark:text-[#F8FAFC]">Error loading</h3>
  ...
  const kpiCards = [
    { label: 'Total Fights', value: stats.total_fights, icon: 'fights', desc: 'Fights registered in the system', color: { bg: 'bg-red-50', text: 'text-red-700', top: 'border-t-[3px] border-t-red-500' }, trend: null },
    { label: 'Active Fights', value: stats.active_fights, icon: 'active', desc: 'Currently in progress', color: { bg: 'bg-emerald-50', text: 'text-emerald-700', top: 'border-t-[3px] border-t-emerald-500' }, trend: null },
    ...
    <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm dark:bg-[#111827] dark:border-[#1E293B]">
                ...
                <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-widest dark:text-[#94A3B8]">System Active</span>
              </div>
      ...
      {isEmpty ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-bold text-slate-800 mb-2 dark:text-[#F8FAFC]">No data available</h3>
          <p className="text-sm text-slate-400 mb-6 dark:text-slate-500">No fights or judges have been registered in the system yet.</p>
          {user?.role !== 'judge' && (
            <button
              onClick={() => navigate('/fights/create')}
              className="px-5 py-2.5 text-sm font-bold text-white bg-wbo-700 rounded-xl hover:bg-wbo-800 transition-all shadow-sm hover:shadow-md active:scale-[0.98]"
          ...
          {user?.role === 'judge' && (
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Scoring</p>
                  <h3 className="text-lg font-bold text-slate-900">My active fights</h3>
                </div>
              </div>
              {activeFightsLoading ? (
                ...
                <div className="py-8 text-center">
                  ...
                  <p className="text-sm font-semibold text-slate-700 mb-1">You have no active fights</p>
                  <p className="text-xs text-slate-400">When a fight you confirmed becomes active, it will appear here.</p>
                </div>
              ) : (
                      ...
                      <div key={a.fight_id} className="flex items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 hover:bg-red-50 transition-colors">
                        ...
                        {isScored ? (
                          <div className="shrink-0 text-right">
                            ...
                            <p className="text-[10px] text-slate-400 mt-0.5">Waiting for the other judges.</p>
                          </div>
                        ) : (
                          <button
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
            ...
            <div className="xl:col-span-4 space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-6 transition-all duration-300 hover:shadow-md dark:bg-[#111827] dark:border-[#1E293B]">
                <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1 dark:text-[#94A3B8]">Quick Actions</p>
                <h3 className="text-lg font-bold text-slate-900 mb-4 dark:text-[#F8FAFC]">Operations</h3>
                <div className="grid grid-cols-1 gap-3">
                  {user?.role !== 'judge' && (
                    <QuickActionCard
                      ...
                      title="New Fight"
                      desc="Create a new fight in the system"
                      onClick={() => navigate('/fights/create')}
                    />
                  )}
                  {user?.role !== 'judge' && (
                  ...
                  {user?.role !== 'judge' && (
                    <QuickActionCard
                      ...
                      title="View Judges"
                      desc="Manage the panel of judges"
                  ...
                  {user?.role !== 'judge' && (
                    <QuickActionCard
                      ...
                      desc="Judge analysis and performance"
            ...
            <div className="xl:col-span-8 space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 dark:bg-[#111827] dark:border-[#1E293B]">
                <div className="px-6 pt-5 pb-3 border-b border-slate-100 flex items-center justify-between dark:border-[#1E293B]">
                  <div>
                    <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1 dark:text-[#94A3B8]">Recent Activity</p>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-[#F8FAFC]">Latest Fights</h3>
                  </div>
                  {user?.role !== 'judge' && (
                    <button
                      onClick={() => navigate('/fights')}
                ...
                {recent_fights.length === 0 ? (
                  <div className="py-10 text-center">
                    <p className="text-sm text-slate-400">No fights have been registered yet.</p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-slate-50/80 dark:bg-[#0B1120]/80">
                        <tr className="border-b border-slate-200 dark:border-[#1E293B]">
                          <th className="text-left py-3.5 px-6 text-[11px] font-semibold text-slate-500 uppercase tracking-wide dark:text-[#94A3B8]">Fight</th>
                          <th className="text-left py-3.5 px-6 text-[11px] font-semibold text-slate-500 uppercase tracking-wide dark:text-[#94A3B8] hidden md:table-cell">Date</th>
                          ...
                          <th className="text-center py-3.5 px-6 text-[11px] font-semibold text-slate-500 uppercase tracking-wide dark:text-[#94A3B8] hidden lg:table-cell">Judges</th>
                          <th className="text-left py-3.5 px-6 text-[11px] font-semibold text-slate-500 uppercase tracking-wide dark:text-[#94A3B8]">Status</th>
                        </tr>
                      </thead>
                      <tbody>
                ...
                <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between dark:border-[#1E293B]">
                  <p className="text-xs text-slate-400 dark:text-slate-500">
                    Showing <span className="font-semibold text-slate-600 dark:text-[#94A3B8]">{recent_fights.length}</span> of{' '}
                    <span className="font-semibold text-slate-600 dark:text-[#94A3B8]">{stats.total_fights}</span> fights
                  </p>
                  <p className="text-xs text-slate-300 dark:text-slate-600">Season 2026</p>
                </div>
              </div>
