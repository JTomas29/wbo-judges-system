...
import { InformationCircleIcon, ClipboardDocumentCheckIcon, UsersIcon, ChartBarIcon } from '@heroicons/react/24/outline';
...
const FightAnalysis = () => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="bg-white dark:bg-[#111827] rounded-xl border border-slate-100 dark:border-[#1E293B] shadow-sm p-10 text-center max-w-sm w-full">
          ...
          <p className="text-slate-500 dark:text-[#94A3B8] text-sm m-0">Cargando análisis...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="bg-white rounded-2xl border border-slate-100 dark:border-[#1E293B] shadow-sm px-10 py-12 text-center max-w-md w-full dark:bg-[#111827]">
          ...
          <p className="text-yellow-900 font-medium text-base leading-relaxed dark:text-yellow-300 m-0">{error}</p>
          <button
  ...
  if (!data || !data.fight) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="bg-white rounded-xl border border-slate-100 dark:border-[#1E293B] shadow-sm p-10 text-center max-w-sm w-full dark:bg-[#111827]">
          <p className="text-slate-400 dark:text-[#94A3B8] m-0">No hay datos de análisis disponibles.</p>
          <button
            className="mt-6 px-8 py-2.5 bg-wbo-700 text-white rounded-xl text-sm font-bold shadow-sm hover:bg-wbo-800 hover:shadow-md hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-200"
            onClick={() => navigate(-1)}
          >
  ...
  return (
    <div className="max-w-[1000px] mx-auto space-y-6 animate-fadeIn">
      ...
      <DetailPageHeader
        ...
        description={`${formatDate(fight.scheduled_date)} · ${fight.weight_class || ''}`}
        status="analyzed"
        backFallback="/fights"
      />
      ...
      <DetailSection icon={InformationCircleIcon} title="Información General" description="Detalles del combate analizado">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          <div>
            <label className="block text-xs text-slate-400 uppercase tracking-wider dark:text-[#94A3B8] m-0">Fecha</label>
            <span className="font-semibold text-slate-800 dark:text-[#F8FAFC]">{formatDate(fight.scheduled_date)}</span>
          </div>
          <div>
            <label className="block text-xs text-slate-400 uppercase tracking-wider dark:text-[#94A3B8] m-0">Categoría</label>
            <span className="font-semibold text-slate-800 dark:text-[#F8FAFC]">{fight.weight_class || '—'}</span>
          </div>
          <div>
            <label className="block text-xs text-slate-400 uppercase tracking-wider dark:text-[#94A3B8] m-0">Lugar</label>
            <span className="font-semibold text-slate-800 dark:text-[#F8FAFC]">{fight.venue || '—'}</span>
          </div>
          <div>
            <label className="block text-xs text-slate-400 uppercase tracking-wider dark:text-[#94A3B8] m-0">Estado</label>
      ...
      {official_card && (
        <DetailSection icon={ClipboardDocumentCheckIcon} title="Resultado Oficial" description="Tarjeta oficial de la pelea">
          <div className="grid grid-cols-3 gap-4 max-w-md mb-4">
            <div className="text-center p-4 rounded-xl bg-slate-50 dark:bg-[#1F2937] border border-slate-100 dark:border-[#1E293B]">
              <p className="text-xs text-slate-400 uppercase tracking-wider mb-1 dark:text-[#94A3B8] m-0">Total Rojo</p>
              <p className="text-2xl font-extrabold text-slate-900 dark:text-[#F8FAFC] m-0">{official_card.total_score_red}</p>
            </div>
            <div className="text-center p-4 rounded-xl bg-slate-50 dark:bg-[#1F2937] border border-slate-100 dark:border-[#1E293B]">
              <p className="text-xs text-slate-400 uppercase tracking-wider mb-1 dark:text-[#94A3B8] m-0">Total Azul</p>
              <p className="text-2xl font-extrabold text-slate-900 dark:text-[#F8FAFC] m-0">{official_card.total_score_blue}</p>
            </div>
            <div className="text-center p-4 rounded-xl bg-slate-50 dark:bg-[#1F2937] border border-slate-100 dark:border-[#1E293B]">
              <p className="text-xs text-slate-400 uppercase tracking-wider mb-1 dark:text-[#94A3B8] m-0">Ganador</p>
              <p className="text-sm font-extrabold text-slate-900 dark:text-[#F8FAFC] m-0">{official_card.winner || 'Empate'}</p>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            {official_card.rounds?.map((r) => (
              <div key={r.round_number} className="grid grid-cols-[70px_1fr_1fr] gap-2 items-center px-3 py-2 bg-slate-50 even:bg-white rounded-lg dark:bg-[#1F2937] dark:even:bg-[#111827] border border-slate-100 dark:border-[#1E293B]">
      ...
      {judges_analysis?.map((judge) => (
        <DetailSection key={judge.judge_id} icon={UsersIcon} title={judge.judge_name} description={judge.assignment_type ? assignmentLabel(judge.assignment_type) : undefined}>
          ...
          <div className="grid grid-cols-3 gap-4 max-w-md mb-4">
            <div className="text-center p-3 rounded-lg bg-slate-50 dark:bg-[#1F2937] border border-slate-100 dark:border-[#1E293B]">
              <p className="text-xs text-slate-400 dark:text-[#94A3B8] m-0">Aciertos</p>
              <p className="text-lg font-extrabold text-green-600 dark:text-green-400 m-0">{judge.matches}</p>
            </div>
            <div className="text-center p-3 rounded-lg bg-slate-50 dark:bg-[#1F2937] border border-slate-100 dark:border-[#1E293B]">
              <p className="text-xs text-slate-400 dark:text-[#94A3B8] m-0">Errores</p>
              <p className="text-lg font-extrabold text-red-600 dark:text-red-400 m-0">{judge.errors}</p>
            </div>
            <div className="text-center p-3 rounded-lg bg-slate-50 dark:bg-[#1F2937] border border-slate-100 dark:border-[#1E293B]">
              <p className="text-xs text-slate-400 dark:text-[#94A3B8] m-0">Precisión</p>
              <p className="text-lg font-extrabold text-slate-900 dark:text-[#F8FAFC] m-0">{judge.match_pct != null ? `${judge.match_pct}%` : '—'}</p>
            </div>
          </div>
          ...
          <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 dark:text-[#94A3B8] m-0">Detalle por Round</h4>
              ...
              <tbody>
                {judge.rounds?.map((r) => (
                  <tr key={r.round_number} className="border-b border-slate-50 hover:bg-slate-50 transition-colors dark:border-[#1E293B] dark:hover:bg-[#1A2435]">
                    <td className="py-2 px-3 font-bold text-wbo-700 dark:text-wbo-400">R{r.round_number}</td>
      ...
      {isStaff && consistency?.length > 0 && (
        <DetailSection icon={ChartBarIcon} title="Consistencia entre Jueces" description="Porcentaje de coincidencia">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-wbo-700 text-white">
      ...
      {judges_analysis?.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-100 dark:border-[#1E293B] shadow-sm p-10 text-center dark:bg-[#111827]">
          <p className="text-slate-400 dark:text-[#94A3B8] m-0">No hay análisis disponible para los jueces de esta pelea.</p>
        </div>
      )}
    </div>
  );
