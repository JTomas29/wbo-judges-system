...
const levelBadge = (level) => {
  ...
  return map[level] || { className: 'bg-slate-100 text-slate-500 border border-slate-200', icon: null };
};
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="bg-white rounded-xl shadow-sm p-10 text-center max-w-sm w-full">
          <div className="animate-spin rounded-full h-10 w-10 border-4 border-slate-200 border-t-wbo-700 mx-auto mb-4" />
          <p className="text-slate-500 text-sm">Cargando análisis...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-[1000px] mx-auto space-y-6 animate-fadeIn">
      ...
      <div className="bg-white rounded-xl shadow-sm p-5">
        <div className="flex flex-wrap justify-between items-start gap-3 mb-4">
          <div>
            <h2 className="text-xl font-bold text-slate-900 m-0">{fight.event_name}</h2>
            <p className="text-sm text-slate-400">{fight.boxer_red} vs {fight.boxer_blue}</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          <div>
            <label className="block text-xs text-slate-400 uppercase tracking-wider">Fecha</label>
            <span className="font-semibold text-slate-800">{formatDate(fight.scheduled_date)}</span>
          </div>
          <div>
            <label className="block text-xs text-slate-400 uppercase tracking-wider">Categoría</label>
            <span className="font-semibold text-slate-800">{fight.weight_class || '—'}</span>
          </div>
          <div>
            <label className="block text-xs text-slate-400 uppercase tracking-wider">Lugar</label>
            <span className="font-semibold text-slate-800">{fight.venue || '—'}</span>
          </div>
          <div>
            <label className="block text-xs text-slate-400 uppercase tracking-wider">Estado</label>
            <span className="font-semibold text-slate-800 capitalize">{fight.status}</span>
          </div>
        </div>
      </div>
      ...
      {official_card && (
        <div className="bg-white rounded-xl shadow-sm p-5">
          <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Resultado Oficial</h3>
          <div className="grid grid-cols-3 gap-4 max-w-md mb-4">
            <div className="text-center p-4 rounded-xl bg-slate-50">
              ...
              <p className="text-2xl font-extrabold text-slate-900">{official_card.total_score_red}</p>
            </div>
            <div className="text-center p-4 rounded-xl bg-slate-50">
              <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Total Azul</p>
              <p className="text-2xl font-extrabold text-slate-900">{official_card.total_score_blue}</p>
            </div>
            <div className="text-center p-4 rounded-xl bg-slate-50">
              <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Ganador</p>
              <p className="text-sm font-extrabold text-slate-900">{official_card.winner || 'Empate'}</p>
            </div>
          </div>
      ...
      {judges_analysis?.map((judge) => (
        <div key={judge.judge_id} className="bg-white rounded-xl shadow-sm p-5">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <h3 className="text-lg font-bold text-slate-900 m-0">{judge.judge_name}</h3>
            {judge.level && (
          ...
          <div className="grid grid-cols-3 gap-4 max-w-md mb-4">
            ...
            <div className="text-center p-3 rounded-lg bg-slate-50">
              ...
              <p className="text-lg font-extrabold text-slate-900">{judge.match_pct != null ? `${judge.match_pct}%` : '—'}</p>
            </div>
          </div>
          ...
          {/* Round table */}
          <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Detalle por Round</h4>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {judge.rounds?.map((r) => (
                  <tr key={r.round_number} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    <td className="py-2 px-3 font-bold text-wbo-700">R{r.round_number}</td>
                    <td className="py-2 px-3 text-center font-semibold text-slate-800">{r.official_score_red}</td>
                    <td className="py-2 px-3 text-center font-semibold text-slate-800">{r.official_score_blue}</td>
                    <td className="py-2 px-3 text-center font-semibold text-slate-800">{r.judge_score_red}</td>
                    <td className="py-2 px-3 text-center font-semibold text-slate-800">{r.judge_score_blue}</td>
      ...
      {isStaff && consistency?.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm p-5">
          <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Consistencia entre Jueces</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-wbo-700 text-white">
              ...
              <tbody>
                {consistency.map((c, i) => (
                  <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    <td className="py-2 px-3 font-semibold text-slate-800">{c.judge_a_name}</td>
                    <td className="py-2 px-3 font-semibold text-slate-800">{c.judge_b_name}</td>
                    <td className="py-2 px-3 text-center">
                      <span className="font-bold text-slate-900">{c.match_pct}%</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {judges_analysis?.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm p-10 text-center">
          <p className="text-slate-400">No hay análisis disponible para los jueces de esta pelea.</p>
        </div>
      )}
    </div>
  );
