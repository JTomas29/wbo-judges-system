  ...
  if (!data || data.length < 2) {
    return (
      <div className="flex items-center justify-center h-48 text-sm text-slate-400 dark:text-[#64748B]">
        Se necesitan al menos 2 evaluaciones para mostrar el gráfico
      </div>
    );
  }
    ...
    getRefereeProfile(id, token)
      ...
      .catch((err) => {
        if (err.response?.status === 404) {
          setError('Árbitro no encontrado');
        } else {
    ...
    return (
      <div className="bg-slate-50 dark:bg-[#0B1120] min-h-screen -mx-4 sm:-mx-6 lg:-mx-8 -mt-5 sm:-mt-6 -mb-5 sm:-mb-6 px-4 sm:px-6 lg:px-8 pt-5 sm:pt-6 pb-5 sm:pb-6 flex items-center justify-center">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-slate-500 font-medium dark:text-[#94A3B8]">Cargando perfil...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-slate-50 dark:bg-[#0B1120] min-h-screen -mx-4 sm:-mx-6 lg:-mx-8 -mt-5 sm:-mt-6 -mb-5 sm:-mb-6 px-4 sm:px-6 lg:px-8 pt-5 sm:pt-6 pb-10 sm:pb-12">
      <div className="max-w-[1440px] mx-auto space-y-6">
        ...
        {/* Evolution Chart */}
        <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-[#1E293B] shadow-sm p-6 transition-all duration-300 hover:shadow-md">
          <h3 className="text-base font-bold text-slate-900 dark:text-[#F8FAFC] mb-4">Evolución de Puntaje</h3>
        ...
        <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-[#1E293B] shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md">
          <div className="px-6 pt-5 pb-3 border-b border-slate-100 dark:border-[#1E293B]">
            <h3 className="text-base font-bold text-slate-900 dark:text-[#F8FAFC]">Historial de Evaluaciones</h3>
            <p className="text-xs text-slate-500 dark:text-[#94A3B8] mt-0.5">
              {history.length} evaluación{history.length !== 1 ? 'es' : ''} registrada{history.length !== 1 ? 's' : ''}
            </p>
          </div>
          ...
          {history.length === 0 ? (
            <div className="p-10 text-center">
              <p className="text-sm font-semibold text-slate-500 dark:text-[#94A3B8]">
                Este árbitro no tiene evaluaciones registradas
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-[#1E293B] bg-slate-50/50 dark:bg-[#0F172A]/50">
                    <th className="text-left py-3.5 px-6 text-[10px] font-semibold text-slate-400 dark:text-[#94A3B8] uppercase tracking-wider">Fecha</th>
                    <th className="text-left py-3.5 px-6 text-[10px] font-semibold text-slate-400 dark:text-[#94A3B8] uppercase tracking-wider">Evento</th>
                    ...
                    <th className="text-center py-3.5 px-6 text-[10px] font-semibold text-slate-400 dark:text-[#94A3B8] uppercase tracking-wider">Calificación</th>
                    <th className="text-center py-3.5 px-6 text-[10px] font-semibold text-slate-400 dark:text-[#94A3B8] uppercase tracking-wider hidden md:table-cell">Descuento</th>
                    <th className="text-center py-3.5 px-6 text-[10px] font-semibold text-slate-400 dark:text-[#94A3B8] uppercase tracking-wider">Puntaje Final</th>
                    <th className="text-left py-3.5 px-6 text-[10px] font-semibold text-slate-400 dark:text-[#94A3B8] uppercase tracking-wider hidden lg:table-cell">Comentarios</th>
                  </tr>
                </thead>
                <tbody>
