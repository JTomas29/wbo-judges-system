...
const EditJudge = () => {
  ...
  useEffect(() => {
    ...
    getJudgeById(id, token)
      ...
      .catch((err) => {
        if (err.response?.status === 404) setError('Judge not found');
        else setError(err.response?.data?.message || 'Error loading judge');
        setLoading(false);
      });
  }, [id, token]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Error updating judge');
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Error deleting judge');
      setDeleting(false);
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/40 rounded-xl px-6 py-4 text-sm font-semibold text-red-700 dark:text-red-300">
          You do not have permission to edit judges.
        </div>
      </div>
    );
  }
  ...
  if (!form) return (
    <div className="flex items-center justify-center py-20">
      <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/40 rounded-xl px-6 py-4 text-sm font-semibold text-red-700 dark:text-red-300">Judge not found</div>
    </div>
  );
  ...
  return (
    <FormCard
      title="Edit Judge"
      subtitle="Update the judge's information."
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-5">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 dark:text-[#94A3B8] mb-1.5">Status</label>
        ...
        >
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 -mt-2">Leave the fields empty if you do not want to change the password.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-5">
            <InputField
              name="password"
              label="New Password"
