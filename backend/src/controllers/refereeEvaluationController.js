...
exports.create = async (req, res, next) => {
  try {
    ...
    if (fight.status !== 'analyzed') {
      return res.status(400).json({ message: 'La pelea debe estar en estado "analyzed" para evaluar al árbitro' });
    }
...
exports.getByFight = async (req, res, next) => {
  try {
    ...
    // Admin puede ver en modo lectura
    // Supervisor puede ver
    // Juez no puede ver (esto se valida en el middleware de ruta)
    res.json(evaluation);
  } catch (err) {
    next(err);
  }
