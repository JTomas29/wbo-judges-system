...
exports.assign = async (req, res, next) => {
  try {
    ...
    if (!judge.is_active) return res.status(400).json({ message: 'El juez no estÃ¡ activo' });
    ...
    if (existing) return res.status(400).json({ message: 'El juez ya estÃ¡ asignado a esta pelea' });

    const count = await JudgeAssignment.getCount(id);
    if (count >= 10) return res.status(400).json({ message: 'No se pueden asignar mÃ¡s de 10 jueces a una pelea' });
...
exports.remove = async (req, res, next) => {
  try {
    ...
    if (!assignment) return res.status(404).json({ message: 'AsignaciÃ³n no encontrada' });
...
exports.respond = async (req, res, next) => {
  try {
    ...
    const fight = await Fight.getById(id);
    if (!fight) return res.status(404).json({ message: 'Pelea no encontrada' });
    if (fight.status !== 'pending') return res.status(400).json({ message: 'La pelea ya no estÃ¡ en estado pending' });

    const assignment = await JudgeAssignment.findOne(id, judgeId);
    if (!assignment) return res.status(404).json({ message: 'No tienes una asignaciÃ³n para esta pelea' });

    if (assignment.status !== 'pending') {
      return res.status(409).json({ message: `La asignaciÃ³n ya fue respondida como ${assignment.status}` });
    }

    const updated = await JudgeAssignment.respond(id, judgeId, response);

    // Obtener estado actualizado de la pelea (el trigger pudo cambiarla a active)
